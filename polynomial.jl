...
struct Polynomial
    terms::Vector{Term}
    Polynomial() = new(Vector{Term}())
    Polynomial(h::Vector{Term},dummy::Int) = new(h)
end
...
function leading(p::Polynomial)::Term 
    return p.terms[1]
end
...
"""
pop!(p::Polynomial)= popat!(p.terms,1)
